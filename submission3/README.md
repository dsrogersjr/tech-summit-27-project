...
| **Objective 9** — test guardrails (app) + budgets (all AI resources) | `ai_budget_verification/EVIDENCE.md`, `…/verify_ai_budget.sql`, `…/budget_attribution_by_product.json`, `…/guardrail_test_run.txt` | Guardrail vitest **13/13** (live); live `system.billing.usage`/`system.serving` verification that every AI resource's spend is tracked + budget-reportable, with the serverless-vs-managed-AI policy-attribution scope documented (supersedes the "null = default applies" note). |
| **Objective 10** — executive AI-usage report (coding agent, MCP, app agent) | `executive_report/EXECUTIVE_USAGE_REPORT.md`, `…/ai_usage_by_stream.json`, `…/cost_by_product.json`, `…/serving_usage_by_endpoint.json`, `…/queries.sql` | Exec report built from live system tables: spend by product (≈$47.20) + the three governed AI-usage streams (app agent in-workspace serving usage; coding agent + MCP via the central Unity AI Gateway). |
