...
export async function syncFromDelta(
  ...
  const [positionRows, queueRows, dispositionRows] = await Promise.all([
    execSql<{
      ...
      warehouseId,
      `SELECT payment_id, program, state, payment_amount_usd,
              CAST(queue_date AS STRING) AS queue_date, n_signals,
              array_join(signal_list, ', ') AS signals, risk_level,
    ...
    execSql<{
      ...
      warehouseId,
      `SELECT payment_id, n_signals,
              array_join(signal_list, ', ') AS signal_list,
              improper_payment_exposure_usd, risk_level
       FROM ${fq('openQueue')}`,
    ),
    hasDispositionTable
      ? execSql<{
      ...
      db
        ...
        .values(
          chunk.map((r) => ({
            ...
            state: r.state,
            paymentAmountUsd:
              r.payment_amount_usd === null ? null : Number(r.payment_amount_usd),
      ...
      db
        ...
        .values(
          chunk.map((r) => ({
            ...
            // Heuristic doesn't emit a discrete hold window; default the
            // hold-for-verification story beat to 3 days (72h).
            recommendedHoldHours:
              r.recommended_disposition === 'hold_for_verification' ? 72 : null,
