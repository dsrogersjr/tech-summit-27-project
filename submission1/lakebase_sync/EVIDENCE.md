...
| 6 | The sync is defined as code (DAB or Terraform), not UI-only | `databricks.yml` (DAB) + `lakebase_setup_db.sh` + `app/server/db/sync.ts` | 5-command reproducible deploy (`dab_instructions.md`) | `sync_as_code.md`, `sync_ts_excerpt.ts` |
...
- `lakebase_sync/sync_as_code.md`, `lakebase_sync/sync_ts_excerpt.ts` — row 6
