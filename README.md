...
| **The move** | Della asks *"Payment PAY-0000202 is flagged as likely improper — should we release it, hold it for verification, or refer it to investigation?"* The live TANF payment in Minnesota is **$3,227.73**, with `cross_agency_fraud_flag` + `income_mismatch` and **~$2,582.18 exposure**. Sentinel ranks the three dispositions and recommends a **72-hour hold-for-verification**; she approves a **48-hour hold** before funds go out, protecting **about $1,678 in predicted recovery** |
...
Della Okonkwo (Deputy Commissioner for Program Integrity) opens her queue console and sees the pre-disbursement payments color-coded: **red** for high-risk improper-payment flags (stacked signals: duplicate identity, deceased, income mismatch, cross-agency fraud match) and **amber** for lower-risk or single-signal flags. She asks about the live hero — *"Payment PAY-0000202 has a cross-agency fraud flag and an income mismatch — should we release it, hold it, or investigate?"* — and the assistant investigates over the governed lakehouse, ranks **release / hold-for-verification / refer-to-investigation**, and surfaces the current heuristic's **72-hour hold-for-verification** recommendation. For this $3,227.73 TANF payment in Minnesota, the estimated exposure is ~$2,582.18 and predicted recovery is about $1,678. She approves a 48-hour hold; the disposition writes back to Lakebase so the next examiner pull shows the hold — **an improper payment stopped before funds disburse.** Governed data, a governed recommendation, and a bounded, audited AI assistant, end to end.
...
2. **Ask why** — in the chat dock, ask why Payment PAY-0000202 is flagged; the assistant investigates the `cross_agency_fraud_flag` + `income_mismatch` signals via Genie over the governed lakehouse.
3. **Get the prescription** — the current heuristic ranks release / hold-for-verification / refer-to-investigation and recommends a 72-hour hold-for-verification, with a what-if (what if we release and discover fraud later).
4. **Act** — approve a 48-hour hold → the disposition + a case memo + verification request write back to Lakebase → the queue and KPIs update live. **The payment is stopped before it disburses.**
